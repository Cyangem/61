...
public class Percolation {
    ...
    //to avoid backwash
    private WeightedQuickUnionUF ufExcludedBottom;
    ...
    public Percolation(int N) {
        ...
        ufExcludedBottom = new WeightedQuickUnionUF(N * N + 1);
    }
    ...
    public void open(int row, int col) {
        ...
        if (row == 0) {
            ...
            ufExcludedBottom.union(xyTo1D(row, col), top);
        }
        if (row == size - 1) {
            ...
            if (adjacentRow >= 0 && adjacentRow < size) {
                if (adjacentCol >= 0 && adjacentCol < size) {
                    if (isOpen(adjacentRow, adjacentCol)) {
                        ...
                        ufExcludedBottom.union(xyTo1D(row, col), xyTo1D(adjacentRow, adjacentCol));
                    }
                }
            }
        }
    ...
    public boolean isFull(int row, int col){
        validate(row, col);
        return ufExcludedBottom.connected(xyTo1D(row, col), top);
    }
